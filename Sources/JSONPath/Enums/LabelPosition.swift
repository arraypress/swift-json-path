...
/// Where the label sits relative to the value it describes.
///
/// A label turns the number into a word — `160` into `Unhealthy` — and shows
/// it alongside the value rather than instead of it, which is what separates
/// it from ``Format/status(rules:)``.
public enum LabelPosition: String, CaseIterable, Sendable, Hashable, Codable {
